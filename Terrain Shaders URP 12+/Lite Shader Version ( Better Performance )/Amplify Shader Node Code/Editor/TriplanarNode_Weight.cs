...
{
    [Serializable]
    [NodeAttributes("Triplanar Sample With Weight", "Textures", "Triplanar")]
    public sealed class TriplanarNode_Weight : ParentNode
    {
        [SerializeField]
        ...
        [SerializeField]
        private string m_propertyInspectorName = "Triplanar Sampler With Weight";
        ...
        protected override void CommonInit(int uniqueId)
        {
            ...
            AddInputPort(WirePortDataType.FLOAT, false, "Vertex Weight");
            AddInputPort(WirePortDataType.SAMPLERSTATE, false, "Smaple State");
        ...
        public override string GenerateShaderForOutput(int outputId, ref MasterNodeDataCollector dataCollector, bool ignoreLocalvar)
        {
...
#endif
                    ssTop = m_inputPorts[11].GeneratePortInstructions(ref dataCollector);
            }
            else
            {
                if (m_midTexPort.IsConnected)
            ...
            var weight = m_inputPorts[10].GeneratePortInstructions(ref dataCollector);

            //            Debug.Log(callHeader);

            if (m_selectedTriplanarType == TriplanarType.Spherical)
                call = dataCollector.AddFunctions(callHeader, samplingTriplanar, texTop + ssTop, pos, norm, falloff, tiling, normalScale, topIndex, weight);
            else
                call = dataCollector.AddFunctions(callHeader, samplingTriplanar, texTop + ssTop, texMid + ssMid, texBot + ssBot, pos, norm, falloff, tiling, normalScale, "float3(" + topIndex + "," + midIndex + "," + botIndex + ")", weight);
